...
for(const [id,url] of projects){
  ...
  if(await outcome.isVisible().catch(()=>false)){await outcome.scrollIntoViewIfNeeded();await page.addStyleTag({content:'.site-header,#dialogClose,.pd-section-nav,[data-skip-link]{display:none!important}'});await outcome.screenshot({path:path.join(dir,'outcome-1440.png')})}
...
for(const [id,url] of projects){
  ...
  const outcome=page.locator('[data-outcome-exact-projection="true"]').first();
  if(await outcome.isVisible().catch(()=>false)){await outcome.scrollIntoViewIfNeeded();await page.addStyleTag({content:'.site-header,#dialogClose,.pd-section-nav,[data-skip-link]{display:none!important}'});await outcome.screenshot({path:path.join(dir,'outcome-390.png')})}
