...
async function footerCase(stage, viewport, group) {
  ...
  return isolated(label, viewport, async (page, shot) => {
    ...
    await footer.evaluate(node=>{node.scrollIntoView({block:"end",behavior:"auto"});const ancestors=[];for(let p=node.parentElement;p;p=p.parentElement)ancestors.push(p);const root=ancestors.find(p=>{const cs=getComputedStyle(p);return /(auto|scroll)/.test(cs.overflowY)&&p.scrollHeight>p.clientHeight+1});if(root)root.scrollTop=root.scrollHeight-root.clientHeight}); await twoFrames(page);
