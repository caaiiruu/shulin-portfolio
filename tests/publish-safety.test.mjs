...
for (const [projectId, project] of Object.entries({ ...(content.experiments || {}), ...(content.sideProjects || {}) })) {
  derive(project.hero, projectId, ["hero"]);
  for (const [sectionIndex, section] of (project.presentationSections || []).entries()) {
    for (const [assetIndex, assetId] of (section.assetIds || []).entries()) {
      slots.push({ projectId, slotId: `presentationSections.${sectionIndex}.assetIds.${assetIndex}`, assetId });
    }
  }
}
derive(content.recognitionRegistry, "profile", ["recognitionRegistry"]);
...
test("Manifest contains only runtime records", () => assert.equal(Object.keys(items).length, new Set(slots.map((slot) => slot.assetId)).size + 5));
test("all 127 governed runtime slots resolve to a public asset", () => {
...
test("all 127 governed runtime slots are production", () => {
  assert.equal(slots.filter((slot) => items[slot.assetId].assetStatus === "awaiting-user-asset" && items[slot.assetId].implementationStatus === "placeholder-active").length, 0);
  assert.equal(slots.filter((slot) => items[slot.assetId].assetStatus === "production" && items[slot.assetId].implementationStatus === "real-active").length, 127);
});
