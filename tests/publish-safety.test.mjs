...
test("all 31 slots resolve to a public placeholder", () => {
...
test("all 31 missing slots are placeholder-active", () => assert.equal(slots.filter((slot) => items[slot.assetId].assetStatus === "awaiting-user-asset" && items[slot.assetId].implementationStatus === "placeholder-active").length, 31));
