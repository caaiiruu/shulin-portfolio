...
test('hero motion reuses the complete animated SVG implementation SSOT before certified flame handoff',()=>{
  ...
  assert.match(heroCss,/@media \(max-width: 900px\)[\s\S]*--hero-motion-anchor-x: 55%/);
  assert.doesNotMatch(heroCss,/inset-block-start: 0;/);
  assert.match(heroCss,/@media \(max-width: 760px\)[\s\S]*--hero-motion-anchor-x: 34%;[\s\S]*overflow: visible;/);
  assert.match(heroCss, /@media \(max-width: 430px\)[\s\S]*object-fit: contain;[\s\S]*transform: none;/);
...
test('Hero clip compensation preserves the governed main overflow owner',()=>{
 const base=fs.readFileSync(new URL('../public/site/assets/css/base.css',import.meta.url),'utf8');
 assert.match(base,/main\{overflow:clip\}/);
 assert.doesNotMatch(heroCss,/\.home-page main\s*\{\s*overflow:\s*visible/);
 assert.match(heroCss,/\.home-page > main#main:has\(> \.hero:first-child\)[\s\S]*margin-block-start: calc\(-1 \* var\(--header-height\)\);[\s\S]*padding-block-start: var\(--header-height\);/);
 assert.doesNotMatch(heroCss,/scaleX\(|scaleY\(|Vector_173[2-6]/);
});

test('Mobile artboard uses one uniform source scale',()=>{const w=Number(heroCss.match(/width: calc\(([^ ]+) \* var\(--dimension-1px\)\)/)[1]);const h=Number(heroCss.match(/height: calc\(([^ ]+) \* var\(--dimension-1px\)\)/)[1]);assert.equal(w/1419,h/780);});
