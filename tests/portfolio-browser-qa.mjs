...
if(r1592.researchValues.join('|')!=='2,857|93%|87%'||!uniform(r1592.metricTypography.map(x=>x.valueStyle))||!uniform(r1592.metricTypography.map(x=>x.labelStyle)))failures.push(`${viewport.name} research metric rendered typography mismatch`);
  ...
  if(projectCardVisual.length<3||projectCardVisual.some(x=>x.color!==x.neutral||x.titleGap===null||Math.abs(x.titleGap-16)>.5))failures.push(`${viewport.name} ProjectCard rendered company hierarchy failed: ${JSON.stringify(projectCardVisual)}`);
  ...
  if(domainShared.length<2||domainShared.some(x=>x.insetLeft===null||Math.abs(x.insetLeft)>.5||Math.abs(x.insetRight)>.5||x.overlap!==0||Math.abs(x.titleGap-16)>.5))failures.push(`${viewport.name} Domain ProjectCard media-stack geometry failed: ${JSON.stringify(domainShared)}`);
