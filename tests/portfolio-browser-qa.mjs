...
if(r1592.researchValues.join('|')!=='2,857|93%|87%'||!uniform(r1592.metricTypography.map(x=>x.valueStyle))||!uniform(r1592.metricTypography.map(x=>x.labelStyle)))failures.push(`${viewport.name} research metric rendered typography mismatch`);
  ...
  for(const [projectName,route] of parityRoutes){
    ...
    const ig=await insight.evaluate(node=>{const h=node.querySelector('.voucher-r149-heading'),e=node.querySelector('.voucher-r149-eyebrow'),t=h?.querySelector('h2'),b=h?.querySelector('.voucher-r149-intro'),r=h?.getBoundingClientRect(),s=x=>x?getComputedStyle(x):null;return{heading:{left:r?.left,right:r?.right,width:r?.width},eyebrowAlign:s(e)?.textAlign,titleAlign:s(t)?.textAlign,bodyAlign:s(b)?.textAlign,centered:r?Math.abs((r.left+r.right)/2-innerWidth/2)<=1:false,edgeSafe:r?r.left>=0&&r.right<=innerWidth:false}});
