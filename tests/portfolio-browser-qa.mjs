...
if(r1592.researchValues.join('|')!=='2,857|93%|87%'||!uniform(r1592.metricTypography.map(x=>x.valueStyle))||!uniform(r1592.metricTypography.map(x=>x.labelStyle)))failures.push(`${viewport.name} research metric rendered typography mismatch`);
  ...
  const projectCardVisual=await page.locator(".work-card-v32 .project-context").evaluateAll(nodes=>nodes.slice(0,5).map(metadata=>{const root=metadata.closest(".work-card-v32"),title=root?.querySelector("h2"),context=metadata.querySelector(".company-context-v132"),cs=getComputedStyle(metadata),tr=title?.getBoundingClientRect(),cr=metadata.getBoundingClientRect();return {company:metadata.textContent.trim(),color:cs.color,neutral:getComputedStyle(document.documentElement).getPropertyValue("--color-text-secondary").trim(),titleGap:tr?tr.top-cr.bottom:null,separator:context?getComputedStyle(context,"::before").marginInline:null}}));
