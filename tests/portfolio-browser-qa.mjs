...
for (const viewport of viewports) {
  ...
  if(JSON.stringify(ctbcCertification.evidenceGroups)!==JSON.stringify(["The application had to be structured before it could be simplified","Interruption was part of the normal journey","Entry context changed what users needed first","Guidance had to remain distinct from formal approval"]))failures.push(`${viewport.name} CTBC decision-shaping Evidence mismatch: ${JSON.stringify(ctbcCertification.evidenceGroups)}`);
  if(ctbcCertification.outcomeHeadline!=="Established the 0→1 application model for CTBC’s in-app mortgage self-service journey."||JSON.stringify(ctbcCertification.outcomeCards)!==JSON.stringify(["One staged application model","Resumable progress as a core product behaviour","Multi-party completion within one application"])||!ctbcCertification.outcomeClosing?.includes("post-launch performance is outside the verified scope"))failures.push(`${viewport.name} CTBC contribution-led Outcomes mismatch: ${JSON.stringify(ctbcCertification)}`);
  if(!ctbcCertification.text.includes("Shaped the 0→1 application model")||!ctbcCertification.text.includes("Turned complex requirements into a buildable product direction")||ctbcCertification.text.includes("Existing acquisition entry points")||ctbcCertification.text.includes("Production launch and post-launch measurement"))failures.push(`${viewport.name} CTBC ownership presentation mismatch`);
