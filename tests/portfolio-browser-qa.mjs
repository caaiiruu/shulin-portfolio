...
for(const viewport of viewports){
  ...
  record(profile.highlights.length===3&&profile.highlights.every(item=>item.visible),`${viewport.name} Profile highlights: ${JSON.stringify(profile.highlights)}`);
  ...
  report.viewports[viewport.name]={routes:routeResults,profile,two,one,themes,cta,baseline,lightChrome,daily,experimentIds,domainRepresentatives,consoleErrors,pageErrors,networkErrors,navigationAbortChecks};
  await context.close();
}
await browser.close();
