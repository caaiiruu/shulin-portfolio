...
for (const viewport of viewports) {
  ...
  for (const [index, route] of routes.entries()) {
    ...
    const metrics = await page.evaluate(() => {
      ...
      const transformation=[...document.querySelectorAll('#projectKeyIntervention')].filter(visible),contribution=[...document.querySelectorAll('.contribution-block')].filter(visible),accountability=[...document.querySelectorAll('#systemCaseAccountabilitySection,#voucherOwnershipSection')].filter(visible);
      ...
      return {
        ...
        semanticSeparation:{transformationCount:transformation.length,labels:transformation[0]?[...transformation[0].querySelectorAll('.key-intervention-map__label')].map(node=>node.textContent.trim().toUpperCase()):[],contributionCount:contribution.length,accountabilityCount:accountability.length,transformationImmediatelyBeforeContribution:Boolean(transformation[0]&&contribution[0]&&transformation[0].parentElement===contribution[0].parentElement&&transformation[0].nextElementSibling===contribution[0]),contributionContainsTransformation:Boolean(contribution[0]?.querySelector('#projectKeyIntervention,.key-intervention-map')),accountabilityNestedInContribution:Boolean(contribution[0]?.querySelector('#systemCaseAccountabilitySection,#voucherOwnershipSection'))}
      };
    });
    ...
    if(projectId&&primaryIds.includes(projectId)){
      ...
      if(semantic.contributionCount!==1||semantic.accountabilityCount!==1||!semantic.transformationImmediatelyBeforeContribution||semantic.contributionContainsTransformation||semantic.accountabilityNestedInContribution)failures.push(`${viewport.name} ${projectId} Transformation / Contribution / Accountability separation failed: ${JSON.stringify(semantic)}`);
  ...
  const expectedTaishinSections=["what-made-this-hard","transformation","contribution","core-system-insight","key-design-decisions","evidence","outcomes","my-accountability","continue-exploring"];
  ...
  const expectedCathaySections=["what-made-this-hard","transformation","contribution","core-system-insight","key-design-decisions","evidence","outcomes","my-accountability","continue-exploring"];
  ...
  if(cathayPresentation.evidenceAssets.length!==4||cathayPresentation.evidenceAssets.some((image,index)=>image.placeholder!=="real-active"||!image.complete||!image.naturalWidth||!image.src?.includes(index===0?"/site/assets/projects/cathay-sit-review-remediation-operations/":"/site/assets/projects/cathay-review/")))failures.push(`${viewport.name} Cathay public-safe Evidence assets failed: ${JSON.stringify(cathayPresentation.evidenceAssets)}`);
  ...
  const cathayOaPresentation=await page.evaluate(()=>{
    ...
    return{
      ...
      outcomeCards:[...dialog.querySelectorAll("#systemCaseOutcomesSection .outcome-semantic-change__item")].filter(visible).map(node=>node.textContent.trim()),
  ...
  const expectedOaSections=["what-made-this-hard","transformation","contribution","core-system-insight","key-design-decisions","evidence","outcomes","my-accountability","continue-exploring"];
  ...
  if(cathayOaPresentation.outcomeValues.length||!cathayOaPresentation.outcomeHeadline||cathayOaPresentation.outcomeCards.length<2)failures.push(`${viewport.name} Cathay OA governed qualitative Outcomes failed: ${JSON.stringify(cathayOaPresentation)}`);
  ...
  if(!cathayOaPresentation.text.includes("implementation and launch remained client-owned")||/\bshipped\b|conversion uplift|completion-rate uplift/i.test(cathayOaPresentation.text))failures.push(`${viewport.name} Cathay OA delivery boundary missing or overstated`);
  ...
  if(bookingCertification.contribution.count!==1)failures.push(`${viewport.name} Booking contribution count failed: ${JSON.stringify(bookingCertification.contribution)}`);
  ...
  if(viewport.width>600&&(bookingCertification.complexity.length!==3||Math.abs(bookingCertification.complexity[1].height-bookingCertification.complexity[2].height)>.75))failures.push(`${viewport.name} Booking complexity equal-height failed: ${JSON.stringify(bookingCertification.complexity)}`);
  ...
  if(ctbcCertification.evidenceGroups.length!==3
    ...
    ||!/three entry contexts|entry model/i.test(`${applicationModelEvidence?.body||""} ${applicationModelSignals}`)
    ||!/staged information|application model/i.test(`${applicationModelEvidence?.body||""} ${applicationModelSignals}`)
    ||!/persistent progress|saved progress|resume/i.test(`${applicationModelEvidence?.body||""} ${applicationModelSignals}`)
  ...
  for(const x of [dbs,voucherCard]){assert(Math.abs(x.frameRatio-16/9)<.02,`${x.projectId} shared 16:9 frame failed`);assert(x.objectFit==="contain",`${x.projectId} shared containment failed`)}
  assert(dbs.mediaFormat==="standard","dbs standard source classification failed");assert(voucherCard.mediaFormat==="standard","voucher approved 16:9 source classification failed");
  ...
  if(JSON.stringify(navigatorContract.labels)!==JSON.stringify(["Overview","Complexity","Decisions","Evidence","Outcomes","Ownership"]))failures.push(`${viewport.name} DBS Project navigator labels mismatch: ${JSON.stringify(navigatorContract.labels)}`);
  ...
  for(const label of ["Ownership","Outcomes","Evidence","Decisions","Complexity","Overview"])await certifyNavigatorClick(label,navigatorInteractions.reverseClicks);
  const ownershipLink=navigator.getByRole("link",{name:"Ownership",exact:true});
  await ownershipLink.click();await ownershipLink.click();await page.waitForTimeout(80);
  navigatorInteractions.repeated=(await ownershipLink.getAttribute("aria-current"))==="location";
...
researchValues: metricTypography.map(item=>item.value),
      ...
      tooltipBoxes:triggers.map(node=>{const glyph=node.querySelector('.info-tooltip__glyph');return {...box(node),glyph:glyph?box(glyph):null,glyphCentered:glyph?Math.abs((box(node).left+box(node).right)/2-(box(glyph).left+box(glyph).right)/2)<1&&Math.abs((box(node).top+box(node).bottom)/2-(box(glyph).top+box(glyph).bottom)/2)<1:false}}),
      horizontalOverflow: document.documentElement.scrollWidth > document.documentElement.clientWidth
    };
  });
...
if(r1592.researchValues.join('|')!=='2,857|93%|87%'||!uniform(r1592.metricTypography.map(x=>x.valueStyle))||!uniform(r1592.metricTypography.map(x=>x.labelStyle)))failures.push(`${viewport.name} research metric rendered typography mismatch`);
  if(r1592.tooltipBoxes.some(box=>Math.abs(box.width-box.height)>1||Math.abs(box.width-48)>1||Math.abs(box.height-48)>1))failures.push(`${viewport.name} Tooltip trigger does not meet the 48px control contract: ${JSON.stringify(r1592.tooltipBoxes)}`);
  if(r1592.tooltipBoxes.some(box=>!box.glyph||Math.abs(box.glyph.width-16)>1||Math.abs(box.glyph.height-16)>1||!box.glyphCentered))failures.push(`${viewport.name} Tooltip Info glyph geometry failed: ${JSON.stringify(r1592.tooltipBoxes)}`);
  if(viewport.width===430){const boxes=r1592.metricTypography.map(x=>x.box);if(boxes.length!==3||boxes.some((b,i)=>i&&Math.abs(b.left-boxes[0].left)>2)||!(boxes[1].top>boxes[0].bottom&&boxes[2].top>boxes[1].bottom)||boxes.some(b=>b.width<200))failures.push(`${viewport.name} research metric rendered rows invalid: ${JSON.stringify(boxes)}`);}
  ...
  await page.goto(`${baseUrl}/site/work/dbs`,{waitUntil:"networkidle"});
  const beforeAfter=await page.locator("[data-component-owner='KeyInterventionMap'] .key-intervention-map__node:is(.is-before,.is-after):visible").evaluateAll(nodes=>nodes.map(node=>({label:node.textContent.trim(),box:(()=>{const r=node.getBoundingClientRect();return {left:r.left,right:r.right,top:r.top,bottom:r.bottom}})()})));
  if(beforeAfter.length!==2||beforeAfter.some(item=>!item.label))failures.push(`${viewport.name} shared Transformation Before/After nodes mismatch: ${JSON.stringify(beforeAfter)}`);
  if(beforeAfter.length===2)await page.locator("[data-component-owner='KeyInterventionMap'] .key-intervention-map__flow").screenshot({path:path.join(targetedDirectory,"r1593-transformation-flow.png")});
  ...
  r158.contribution = await page.locator(".contribution-block").evaluate(section => {
    const cards=[...section.querySelectorAll(".voucher-r149-rows--teams>div")].map(node=>{const r=node.getBoundingClientRect();return {x:r.x,y:r.y,width:r.width,height:r.height,background:getComputedStyle(node).backgroundColor,color:getComputedStyle(node).color}});
    return {cards,standalone:section.dataset.canonicalSectionId==="my-contribution",count:cards.length};
  });
  ...
  if(viewport.width<=430&&(r158.cloud.leftDelta>1||r158.cloud.rightDelta>1))failures.push(`${viewport.name} Core System Insight cloud did not reach dialog edges: ${JSON.stringify(r158.cloud)}`);
  ...
  const r171Order=["what-made-this-hard","transformation","contribution","core-system-insight","key-design-decisions","evidence","outcomes","my-accountability"];
  if(r171Presentation.title!=="Uncertain expansion to a lower-risk taxi pickup experiment"||r171Presentation.taxonomy.length||JSON.stringify(r171Presentation.navigator)!==JSON.stringify(["Overview","Complexity","Decisions","Evidence","Outcomes","Ownership"])||JSON.stringify(r171Presentation.order.filter(id=>r171Order.includes(id)))!==JSON.stringify(r171Order)||!(r171Presentation.decisionTop<r171Presentation.evidenceTop)||r171Presentation.decisionCount!==3||r171Presentation.evidenceCount!==6)failures.push(`${viewport.name} R171 composition mismatch: ${JSON.stringify(r171Presentation)}`);
  ...
  if([1419,871,430].includes(viewport.width)){
    ...
    const checkpoints=[["01-overview","#projectOverviewSection","#projectOverviewSection"],["02-complexity","#systemCaseComplexitySection","#systemCaseComplexitySection"],["03-decisions","#systemCaseDecisionsSection","#systemCaseDecisionsSection"],["04-evidence-a","#systemCaseEvidenceSection .structured-evidence-v223__group:first-of-type","#systemCaseEvidenceSection"],["05-evidence-b","#systemCaseEvidenceSection .structured-evidence-v223__group:last-of-type","#systemCaseEvidenceSection"],["07-ownership","#systemCaseAccountabilitySection","#systemCaseAccountabilitySection"]];
    const scroll=page.locator("#detailDialog .dialog-scroll").first();
    for(const [name,selector,expectedHref] of checkpoints){
  ...
  const r172Order=["what-made-this-hard","transformation","contribution","core-system-insight","key-design-decisions","evidence","outcomes","my-accountability"];
  ...
  if(r172Presentation.atAGlance!=="Led UX redesign of a launched mortgage consultation tool, replacing a fixed tablet script with scenario-led guidance validated across 4 core tasks.")failures.push(`${viewport.name} R172.3 At a Glance mismatch`);
  ...
  await page.goto(`${baseUrl}/site/profile.html`,{waitUntil:'networkidle'});
  const profileVisual=await page.evaluate(()=>{const visible=node=>Boolean(node&&getComputedStyle(node).display!=='none'&&node.getClientRects().length),rect=node=>{const r=node.getBoundingClientRect();return{left:r.left,right:r.right,top:r.top,bottom:r.bottom,width:r.width,height:r.height,centerY:r.top+r.height/2}},ctas=[...document.querySelectorAll('.timeline-evidence-v34')].filter(visible).map(card=>{const arrow=card.querySelector(':scope>.icon-arrow'),copy=card.querySelector('.timeline-evidence-v34__copy'),cr=rect(card),ar=rect(arrow),xr=rect(copy);return{card:cr,arrow:ar,copy:xr,rightGap:cr.right-ar.right,verticalDelta:Math.abs(cr.centerY-ar.centerY),arrowAfterCopy:ar.left>=xr.right,fullCardButton:card.tagName==='BUTTON'}}),awardCards=[...document.querySelectorAll('#profileSideRail>article')].map(card=>({text:card.innerText,logo:card.querySelector('[data-award-logo="red-dot"] img')?{src:card.querySelector('[data-award-logo="red-dot"] img').getAttribute('src'),box:rect(card.querySelector('[data-award-logo="red-dot"] img')),natural:[card.querySelector('[data-award-logo="red-dot"] img').naturalWidth,card.querySelector('[data-award-logo="red-dot"] img').naturalHeight]}:null,placeholder:/^(RD|GP)$/.test(card.querySelector('.award-logo-slot')?.textContent.trim()||'')}));return{ctas,awardCards,overflow:document.documentElement.scrollWidth-document.documentElement.clientWidth}});
  if(profileVisual.ctas.some(item=>!item.fullCardButton||item.verticalDelta>1||!item.arrowAfterCopy||item.rightGap>20))failures.push(`${viewport.name} Profile View Case CTA alignment failed: ${JSON.stringify(profileVisual.ctas)}`);
  if(profileVisual.awardCards.some(card=>/International recognition|Taiwan recognition/.test(card.text)||card.placeholder)||!profileVisual.awardCards[0]?.logo||!profileVisual.awardCards[0].logo.src.endsWith('/site/assets/img/red-dot-award-logo.svg')||profileVisual.awardCards[0].logo.natural.some(value=>!value))failures.push(`${viewport.name} Profile award-card cleanup failed: ${JSON.stringify(profileVisual.awardCards)}`);
  if(profileVisual.overflow>0)failures.push(`${viewport.name} Profile horizontal overflow: ${profileVisual.overflow}`);

  for(const route of ['/site/index.html','/site/work.html','/site/profile.html']){
    await page.goto(`${baseUrl}${route}`,{waitUntil:'networkidle'});
    const exposure=await page.evaluate(()=>({links:document.querySelectorAll('a[href*="experiments"]').length,triggers:document.querySelectorAll('[data-experiment]').length,body:document.body.innerText}));
    const expectedTriggers=route==='/site/profile.html'?1:0;
    if(exposure.links<2||exposure.triggers!==expectedTriggers)failures.push(`${viewport.name} R183.8E governed Experiment discovery mismatch on ${route}: ${JSON.stringify(exposure)}`);
  }
  await page.goto(`${baseUrl}/site/experiments.html`,{waitUntil:'networkidle'});
  const experimentDiscovery=await page.evaluate(()=>({cards:document.querySelectorAll('#experimentPageRail [data-experiment]').length,hidden:document.querySelector('#experimentIndex')?.hidden,runtime:Object.keys(window.PORTFOLIO_RUNTIME_DATA?.experiments||{}).length,overflow:document.documentElement.scrollWidth-document.documentElement.clientWidth}));
  if(experimentDiscovery.cards!==7||experimentDiscovery.hidden||experimentDiscovery.runtime!==7||experimentDiscovery.overflow>0)failures.push(`${viewport.name} R183.8E Experiment route failed: ${JSON.stringify(experimentDiscovery)}`);
