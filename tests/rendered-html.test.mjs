...
test("keeps the homepage Hero as one accessible, responsive owner", () => {
  ...
  assert.match(transformation,/52%\s*\{\s*opacity:\.9;\s*transform:translate\(-17px,210px\) scale\(\.68\);\s*\}[\s\S]*62%\s*\{\s*opacity:\.48/);
  assert.match(transformation,/@keyframes unified-cloud-takeover[\s\S]*50%[\s\S]*58%\s*\{\s*opacity:\.32[\s\S]*86%\s*\{\s*opacity:\.76/);
  assert.match(transformation,/@keyframes flame-reveal[\s\S]*78%[\s\S]*84%\s*\{\s*opacity:\s*\.14[\s\S]*96%\s*\{\s*opacity:\s*\.9/);
