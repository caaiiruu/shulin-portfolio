...
test("keeps the homepage Hero as one accessible, responsive owner", () => {
  ...
  assert.match(hero, /translateX\(calc\(-50% \+ var\(--hero-transformation-shift-mobile\)\)\)/);
  assert.match(hero, /@media \(max-width: 430px\)[\s\S]*\.hero__artwork \{[^}]*inset-inline-start: -10%;[^}]*width: 80%;[^}]*min-width: 0/);
  assert.match(hero, /@media \(max-width: 430px\)[\s\S]*\.hero__transformation \{ transform:translate\(calc\(-50% \+ var\(--dimension-9vw\)\),-13%\)/);
  assert.match(hero, /prefers-reduced-motion: reduce/);
  assert.doesNotMatch(hero, /@keyframes hero-hand-enter/);
  assert.match(hero, /\.hero__transformation \{[\s\S]*inset-inline-start: 50%;[\s\S]*width: var\(--dimension-100vw\);[\s\S]*max-width: none;/);
