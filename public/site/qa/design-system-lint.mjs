...
if (!app.includes("dialogScrollRoot.scrollTo({left:0,top:destination,behavior:prefersReduced.matches?'auto':'smooth'})") || !app.includes("if(projectSectionNavigation){scheduleProjectSectionNavigationCompletion") || !app.includes("setActiveProjectSection(visibleProjectSectionId)") || app.includes("setTimeout(settle,900)")) errors.push("ProjectDetailOverview: navigator click and scroll-spy must share one deterministic motion-aware .dialog-scroll state path");
