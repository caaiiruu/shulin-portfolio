...
if (!projectDetail.includes(".quick-view-v51--project .project-summary-v45{order:1;") || !projectDetail.includes(".quick-view-v51--project .project-signals-v45{order:2}") || !projectDetail.includes(".project-detail-hero-visual{grid-column:1/-1")) errors.push("ProjectDetailOverview: At a Glance and Info Grid must precede the full-width Lead Project Visual");
...
if (!app.includes("function animateProjectSectionNavigation(token)") || !app.includes("function cancelProjectSectionNavigation()") || !app.includes("setActiveProjectSection(visibleProjectSectionId)") || !app.includes("dialogScrollRoot?.addEventListener('wheel',cancelProjectSectionNavigation") || app.includes("setTimeout(settle,900)")) errors.push("ProjectDetailOverview: navigator click, live-target animation, interruption, and scroll-spy must share one deterministic .dialog-scroll state path");
...
if (!experimentCard.includes(".experiment-overview-v45__question,.experiment-overview-v45__build{width:100%;max-width:var(--project-detail-reading-max);min-width:0;padding:0;border-radius:0;background:transparent}")) errors.push("ExperimentDetail: overview content must reuse the governed Project Detail reading width without nested card styling");
