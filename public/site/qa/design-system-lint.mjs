...
if (app.includes("stage-focus-v148__statement") || !app.includes("createProgrammeSection(stageLabel,'')") || !app.includes("const stageProjection=parent.recruiterFirstPopup")) errors.push("ProjectDetailOverview: recruiter-first stage details must use the stage name directly and must not render a Stage Focus wrapper");
if (!app.includes("createDecisionCard(model,index,{showVisual:false})") || !app.includes("voucher-r149-decision-list")) errors.push("ProjectDetailOverview: recruiter-first stage details must reuse the canonical Design Decision renderer");
