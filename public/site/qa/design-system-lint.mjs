...
if (!app.includes("function animateProjectSectionNavigation(token)") || !app.includes("function cancelProjectSectionNavigation()") || !app.includes("if(prefersReduced.matches)") || !app.includes("if(projectSectionNavigation)return") || !app.includes("setActiveProjectSection(visibleProjectSectionId)") || app.includes("setTimeout(settle,900)")) errors.push("ProjectDetailOverview: navigator click and scroll-spy must share one deterministic motion-aware .dialog-scroll state path");
