...
if (!app.includes("const behavior=prefersReduced.matches?'auto':'smooth'") || !app.includes("dialogScrollRoot.scrollTo({left:0,top:destination,behavior})") || !app.includes("history.replaceState(history.state,'',\`#\${target.id}\`)") || !app.includes("setActiveProjectSection(visibleProjectSectionId)") || app.includes("setTimeout(settle,900)")) errors.push("ProjectDetailOverview: navigator must use shared smooth scrolling, reduced-motion fallback, hash preservation and one deterministic .dialog-scroll state path");
