...
Version: 16.3 — v54 world-class recruitment and layout revision
...
- Homepage order is Hero → Experience proof → Selected evidence → Matcher → Domain experience → Principles → Playground → Contact.
- Selected work must show Voucher first, followed by DBS and Booking.com; supporting work is visually subordinate.
- Professional project evidence remains white, precise, and recruiter-scannable.
- Personal brand appears through pale-blue organic fields, irregular chapter edges, purposeful coral accents, and restrained internal motion.
- Organic fields are atmospheric background elements. They must never reduce text contrast, cover controls, or mimic content cards.
- Quick preview opens the accessible evidence dialog; “Full case” links use `work.html?case=<project>` so a case is directly shareable.
- The Domain floating rail is desktop-only and appears only while the reader is inside the Domain chapter. Mobile uses the native picker.
- Unverified award records remain hidden. Side-project recognition is displayed only after its title, organisation, year, and project are verified.
- `assets/css/portfolio-v53.css` is the final style owner and may override v52 styles only to implement this lock.
- No emoji, beige global background, decorative cursor, autoplay carousel, or invented evidence is permitted.

## v54 execution lock

- `assets/css/portfolio-v54.css` is the final layout and responsive owner.
- Every dialog viewport must remain at horizontal position zero; horizontal dialog scrolling is prohibited.
- Desktop dialog content uses a bounded heading, two-column gallery/summary layout, and one-column evidence below 1100px.
- Mobile dialogs use the full viewport, a fixed close control, one-column signals, and no clipped headings or content.
- Global section, card, filter, rail, and supporting-page rhythm uses the v54 spacing scale.
- The first viewport must expose seniority, complex-systems positioning, AI-enabled workflow relevance, and global-remote availability.
- Do not imply verified business impact, AI delivery experience, leadership scope, or awards unless supported by supplied evidence.
...
# 52. V52 FLOATING DOMAIN NAVIGATION, DETAIL RHYTHM, AND RECOGNITION — 2026-07-20

## 52.1 Detail impact rhythm

- Business impact and Team impact use the same outer padding, heading gap, and card radius.
- Team impact items may not stretch to fill unused vertical space.
- Desktop uses three equal, top-aligned items.
- Tablet and mobile use compact stakeholder / change rows.
- Confidentiality is secondary metadata separated by one subtle rule.

## 52.2 Floating Domain navigation

Desktop only:

- The original six-domain index remains the entry control.
- After the reader moves more than 210px into the Domain chapter, the controls transform into a bottom floating horizontal rail.
- The original controls visually recede while the floating rail is active.
- The floating rail disappears when the reader returns to the entry index or exits the Domain chapter.
- Selected state remains synchronised, and the selected chip scrolls into view.

Mobile keeps the existing sticky top chip rail and does not show a second floating rail.

## 52.3 Matcher alignment

- The default title and supporting copy share one centred axis.
- Result mode switches both to left alignment.
- Suggested problem chips use compact 13–14px labels and a 42–44px target.

## 52.4 Contrast ownership

- Cyan experiment cards always use white primary text and high-contrast secondary text.
- Cream cards always use dark text.
- Dark cards always use white text.
- Hover cannot change a card into another semantic colour.

## 52.5 Awards and side projects

Use a project-level award badge when the recognition is tied to a specific side project.

- Keep the badge hidden until award title, organisation, year, and project association are verified.
- The separate Awards section is reserved for recognition not tied to one project or for the complete chronological record.
- Do not duplicate the same award at equal visual weight in both places.
