...
- Professional project cases use one compact bottom-floating anchor navigator derived from the canonical recruiter-first section contract: Overview, Complexity, Decisions, Evidence, Outcomes, and Ownership. It is a direct child of the dialog shell, outside the animated scroll container, so it remains stationary throughout the popup. It is document navigation rather than a tablist; genuinely missing sections are omitted without hiding the remaining navigator. Interactive activation uses the shared live-target smooth-anchor owner, while initial semantic hash loading resolves immediately to the same canonical target.
