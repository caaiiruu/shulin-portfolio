(() => {
  ...
  const buildVisual = (project, key) => {
    ...
    const csv2Asset = project?.presentationContract && project.presentationContract !== 'legacy' ? window.CASE_STUDY_ASSETS?.[key]?.assets?.[assetId] : null;
  ...
  const buildCard = (key) => {
    const project = PUBLIC_PROJECTS[key] || {};
    ...
    type.textContent = readType(project) || (project.presentationContract && project.presentationContract !== 'legacy' ? firstText(project.company) : '');
    const identity = document.createElement('span');
    identity.className = 'domain-project-card-v2__identity';
    const company = document.createElement('strong');
    company.className = 'domain-project-card-v2__company';
    company.textContent = project.presentationContract && project.presentationContract !== 'legacy' ? 'Independent' : firstText(project?.company);
    ...
    cta.href = project.publicRoute || `/work/${key}`;
  ...
  const navigateCard = (card) => { const key = card?.dataset.project; const project = key ? PUBLIC_PROJECTS[key] : null; if (key) window.location.href = project?.publicRoute || `/work/${key}`; };
