(function(){
 ...
 const reusable=section('',t(c.reusableSystem?.title)),found=element('div','voucher-r149-foundations');list(c.reusableSystem?.foundations).forEach(x=>{const r=element('article','voucher-r149-foundation'),media=element('div','voucher-r149-foundation__media');const asset=resolveProjectAsset(x.assetId||'project-visual-placeholder-square-v1'),img=doc.createElement('img');img.src=asset.src;img.alt=localize(asset.alt);img.loading='lazy';media.append(img);r.append(media,element('h3','',t(x.label)),element('p','',t(x.text)));found.append(r)});reusable.append(found);
 const research=c.reusableSystem?.voucherCard?.researchExample||{};const rs=element('section','voucher-r149-subsection voucher-r149-research voucher-r149-voucher-card-integrated');rs.append(element('h3','',t(c.reusableSystem?.voucherCard?.headline)),element('p','',t(c.reusableSystem?.voucherCard?.history)),element('h4','',t(research.title)),element('p','voucher-r149-intro',t(research.supporting)),element('small','voucher-r149-study',t(research.study)));const vm=element('div','voucher-r149-findings');list(c.reusableSystem?.voucherCard?.metrics).forEach((x,index)=>{const d=element('article');d.append(element('span','voucher-r149-finding-number',String(index+1).padStart(2,'0')),element('strong','',x.value),element('p','',t(x.label)));vm.append(d)});const third=element('article');third.append(element('span','voucher-r149-finding-number','03'),element('p','',t(research.thirdFinding)));vm.append(third);rs.append(vm,element('p','voucher-r149-credibility',t(research.credibility)));reusable.append(rs);
 const future=element('section','voucher-r149-subsection voucher-r149-future-integrated');future.append(element('h3','',t(c.reusableSystem?.future?.headline)),element('p','',t(c.reusableSystem?.future?.thesis)));const fr=element('div','voucher-r149-rows');list(c.reusableSystem?.future?.items).forEach(x=>{const r=element('div');r.append(element('strong','',t(x.label)),element('p','',t(x.content)));fr.append(r)});future.append(fr);reusable.append(future);
