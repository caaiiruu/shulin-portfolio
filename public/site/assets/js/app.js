(function(){
 ...
 list(c.stages).forEach((s,i)=>{
   ...
   const overview=s.overview||{keyInsight:s.keyInsight,designDirection:s.whatChanged,designSolution:s.shipped,effect:s.shipped};
   [[lang==='zh'?'關鍵洞察':'KEY INSIGHT',overview.keyInsight],[lang==='zh'?'設計方向':'DESIGN DIRECTION',overview.designDirection],[lang==='zh'?'設計方案':'DESIGN SOLUTION',overview.designSolution],[lang==='zh'?'效果':'EFFECT',overview.effect]].forEach(([l,x])=>{const d=element('div');d.append(element('span','voucher-r149-eyebrow',l),element('p','',t(x)));facts.append(d)});
 ...
 const research=c.reusableSystem?.voucherCard?.researchExample||{};const rs=element('section','voucher-r149-subsection voucher-r149-research voucher-r149-voucher-card-integrated');rs.append(element('h3','',t(c.reusableSystem?.voucherCard?.headline)),element('p','voucher-r149-intro',t(research.supporting)));const vm=element('div','voucher-r149-findings impact-evidence-v147__metrics');list(c.reusableSystem?.voucherCard?.metrics).forEach(x=>{const d=element('article','impact-evidence-v147__metric');d.append(element('strong','impact-evidence-v147__value',x.value),element('span','impact-evidence-v147__label',t(x.label)));vm.append(d)});const third=element('article','impact-evidence-v147__metric');third.append(element('strong','impact-evidence-v147__value',lang==='zh'?'色彩':'Colour'),element('span','impact-evidence-v147__label',t(research.thirdFinding)));vm.append(third);rs.append(vm,element('p','voucher-r149-research__bridge',t(c.reusableSystem?.voucherCard?.bridge)),element('p','voucher-r149-credibility',t(research.credibility)));reusable.append(rs);
