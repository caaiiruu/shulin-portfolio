(function(){
 ...
 const contribution=section('',lang==='zh'?'貢獻':'Contribution','',t(c.contribution?.title));contribution.classList.add('contribution-block','contribution-block--emphasis');contribution.dataset.componentOwner='ContributionBlock';const flow=element('div','voucher-r149-flow');list(c.contribution?.transformation).forEach((x,i)=>{const a=element('article');a.append(element('span','voucher-r149-eyebrow',t(x.label)),element('p','',t(x.text)));flow.append(a);if(i<2)flow.append(element('i','','→'))});const teams=element('div','voucher-r149-rows voucher-r149-rows--teams');teams.append(element('h3','',lang==='zh'?'跨團隊啟用':'Enabled across teams'));list(c.contribution?.teams).forEach(x=>{const r=element('div');r.append(element('strong','',t(x.label)),element('p','',t(x.text)));teams.append(r)});contribution.append(flow,teams);
 ...
 list(c.stages).forEach((s,i)=>{
   ...
   const b=element('a','button button--dark programme-stage-case__cta',lang==='zh'?'查看解決方案細節 →':'View solution details →');const stageUrl=new URL(window.location.href);stageUrl.searchParams.set('case',key);stageUrl.searchParams.set('stage',s.id);stageUrl.searchParams.delete('initiative');b.href=stageUrl.pathname+stageUrl.search+stageUrl.hash;b.dataset.stage=s.id;b.dataset.parentProject=key;b.setAttribute('aria-haspopup','dialog');
 ...
 const voucherCard=c.reusableSystem?.voucherCard||{},research=voucherCard.researchExample||{};const rs=element('section','voucher-r149-subsection voucher-r149-research voucher-r149-voucher-card-integrated');rs.append(element('h3','',t(voucherCard.headline)),element('p','voucher-r149-intro',t(research.supporting)));const vm=element('div','voucher-r149-findings impact-evidence-v147__metrics');list(voucherCard.evidence).forEach(x=>{const d=element('article','impact-evidence-v147__metric');d.append(element('strong','impact-evidence-v147__value',x.value),element('span','impact-evidence-v147__label',t(x.label)));if(t(x.detail))d.append(element('span','impact-evidence-v147__detail',t(x.detail)));vm.append(d)});rs.append(vm,element('p','voucher-r149-research__supporting',t(voucherCard.supportingEvidence)),element('p','voucher-r149-research__bridge',t(voucherCard.bridge)),element('p','voucher-r149-credibility',t(voucherCard.credibilityNote||research.credibility)));reusable.append(rs);
 ...
 const programmeResearchSection=section('',t(c.programmeResearch?.title),t(c.programmeResearch?.summary));programmeResearchSection.dataset.componentOwner='ResearchEvidenceMetric';const rm=element('div','research-evidence-metrics');list(c.programmeResearch?.metrics).forEach(x=>{const m=element('article','research-evidence-metric'),label=element('span','research-evidence-metric__label',t(x.label));m.append(element('strong','',x.value),label);const tip=createInfoTooltip(t(x.note),lang==='zh'?'查看研究證據':'View research evidence');if(tip)label.append(tip);rm.append(m)});programmeResearchSection.append(rm);const outcomes=section('',lang==='zh'?'成果':'Outcomes','',t(c.outcomes?.title)),metrics=element('div','voucher-r149-metrics outcome-metric-grid');outcomes.id='voucherImpactSection';outcomes.dataset.projectNavTarget='impact';outcomes.dataset.componentOwner='OutcomeMetric';list(c.outcomes?.metrics).forEach(x=>{const m=element('article','outcome-metric'),primary=element('div','outcome-metric__primary'),copy=t(x.primaryCopy)||t(x.label);primary.append(element('span','outcome-metric__label',copy));const tip=createInfoTooltip(t(x.evidenceNote)||t(x.label),lang==='zh'?'查看成果證據':'View outcome evidence');if(tip)primary.append(tip);m.append(element('strong','outcome-metric__value',x.value),primary);metrics.append(m)});outcomes.append(metrics,element('p','voucher-r149-system-change',t(c.outcomes?.systemChange)));
  ...
  function renderDetail(){
    ...
    if(isStage){
      ...
      list(stageProjection?.decisions).forEach((source,index)=>{
        ...
        card.classList.add('voucher-r149-decision','decision-module');card.dataset.componentOwner='DecisionModule';
