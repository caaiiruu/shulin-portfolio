(function(){
 ...
 safeText(doc.getElementById('detailTaxonomyLabel'),ui("problem-types-dd6fa301"));
  ...
  const si=p.publicContent?.coreSystemInsight;if(si){const n=createProgrammeSection(localize(si.eyebrow),localize(si.title),'core-system-insight-section');n.dataset.componentOwner='CoreSystemInsightSection';n.append(element('p','',localize(si.statement)));programmeSurface().append(n)}const fo=p.publicContent?.futureServiceOpportunities;if(fo){const n=createProgrammeSection('',localize(fo.title),'future-service-opportunities'),g=element('div','future-service-opportunities__grid');list(fo.items).forEach(x=>{const m=element('article','decision-module');m.append(element('h3','',localize(x.title)),element('p','',localize(x.concept)),element('p','',localize(x.whyItMatters)),element('p','',localize(x.systemRequirement)));g.append(m)});n.append(g);programmeSurface().append(n)}}
 ...
 const programmeResearchSection=section('',t(c.programmeResearch?.title),t(c.programmeResearch?.summary));programmeResearchSection.dataset.componentOwner='ResearchEvidenceMetric';const rm=element('div','research-evidence-metrics');list(c.programmeResearch?.metrics).forEach(x=>{const m=element('article','research-evidence-metric');m.append(element('strong','',x.value),element('span','',t(x.label)));if(t(x.note))m.append(element('small','',t(x.note)));rm.append(m)});programmeResearchSection.append(rm);const outcomes=section('',lang==='zh'?'成果':'Outcomes','',t(c.outcomes?.title)),metrics=element('div','voucher-r149-metrics outcome-metric-grid');outcomes.id='voucherImpactSection';outcomes.dataset.projectNavTarget='impact';outcomes.dataset.componentOwner='OutcomeMetric';list(c.outcomes?.metrics).forEach(x=>{const m=element('article','outcome-metric');m.append(element('strong','outcome-metric__value',x.value),element('span','',t(x.label)));if(t(x.primaryCopy))m.append(element('p','',t(x.primaryCopy)));if(t(x.evidenceNote))m.append(element('small','',t(x.evidenceNote)));metrics.append(m)});outcomes.append(metrics,element('p','voucher-r149-system-change',t(c.outcomes?.systemChange)));
