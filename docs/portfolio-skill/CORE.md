...
15. Core System Insight is insight-first, not artifact-first or metric-first. Use the cognitive sequence `INSIGHT → WHAT THIS CHANGED → VISUAL PROOF` within the centered readable composition. Visual proof may be a simplified journey map, system/state/transaction model, architecture, or before/after transformation; never mandate one artifact type.
